...
if (!class_exists('QRcode', false)) {
	...
	class QRcode {
		 ...
		 */
		 protected function estimateVersion($items) {
			...
			do {
				...
				$version = $this->getMinisumVersion((int)(($bits + 7) / 8), $this->level);
				if ($version < 0) {
					return -1;
				}
			} while ($version > $prev);
		 ...
		 */
		 protected function convertData($items) {
			...
			for (;;) {
				...
				$ver = $this->getMinisumVersion((int)(($bits + 7) / 8), $this->level);
				if ($ver < 0) {
					return -1;
		 ...
		 */
		protected function getMinisumVersion($size, $level) {
